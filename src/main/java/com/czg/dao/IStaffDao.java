...
public interface IStaffDao {
    ...
    @Results(//结果集
            ...
            value = {
                    ...
                    @Result(column = "dept_id", property = "dept", one = @One(select = "com.czg.dao.IStaffDao.selectDeptById", fetchType = FetchType.LAZY))
            }
    )
    @Select("SELECT STAFF_ID, STAFF_NAME, DEPT_ID, STAFF_WORKCODE, STAFF_PASSWORD " +
    ...
    @Select("SELECT STAFF_ID, STAFF_NAME, DEPT_ID, STAFF_WORKCODE, STAFF_PASSWORD" +
            " FROM TASK_STAFF WHERE DEPT_ID = #{dept_id}")
    @ResultMap("selectStaff")
